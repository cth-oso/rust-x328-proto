...
impl Receiver<ReadResult> for ReceiveReadResponse<'_> {
    fn receive_data(mut self, data: &[u8]) -> ReceiveDataProgress<Self, ReadResult> {
        ...
        ReceiveDataProgress::Done(match parse_read_response(self.buffer.as_ref()) {
            ...
            ResponseToken::InvalidParameter => Err(Error::InvalidParameter),
            _ => Err(Error::ProtocolError),
        })
    }
}
...
pub mod io {
    ...
    use crate::master::{self, ReceiveDataProgress, Receiver, SendData};
    ...
    fn check_addr_param(
        ...
        Ok((addr.into_address()?, param.into_parameter()?))
    }
} // mod io
